...
.feat-items{{margin-top:10px;display:flex;flex-direction:column;gap:4px}}
.feat-item{{font-size:11px;display:flex;align-items:center;gap:6px}}
.feat-card-soil .feat-item{{color:rgba(255,255,255,.45)}}
.feat-card-env .feat-item{{color:#3B6D11}}
.feat-card-extra .feat-item{{color:#854F0B}}
.feat-item-dot{{width:4px;height:4px;border-radius:50%;flex-shrink:0}}
.feat-card-soil .feat-item-dot{{background:rgba(168,212,74,.4)}}
.feat-card-env .feat-item-dot{{background:#7CB518}}
.feat-card-extra .feat-item-dot{{background:#BA7517}}
</style>
</head>
<body>
<div class="app">
        ...
        <div class="feat-grid">
          ...
          <div class="feat-card feat-card-soil">
            ...
            <div class="feat-items">
              <div class="feat-item"><div class="feat-item-dot"></div>Bulk Density</div>
              <div class="feat-item"><div class="feat-item-dot"></div>Organic Matter</div>
              <div class="feat-item"><div class="feat-item-dot"></div>Salinity & CEC</div>
            </div>
          </div>
          <!-- Card 2: light green -->
          <div class="feat-card feat-card-env">
            ...
            <div class="feat-items">
              <div class="feat-item"><div class="feat-item-dot"></div>Suhu udara & tanah</div>
              <div class="feat-item"><div class="feat-item-dot"></div>Soil Moisture</div>
              <div class="feat-item"><div class="feat-item-dot"></div>Light Intensity</div>
            </div>
          </div>
          <!-- Card 3: warm cream -->
          <div class="feat-card feat-card-extra">
            ...
            <div class="feat-items">
              <div class="feat-item"><div class="feat-item-dot"></div>Nutrient Balance</div>
              <div class="feat-item"><div class="feat-item-dot"></div>Moisture Regime</div>
              <div class="feat-item"><div class="feat-item-dot"></div>Thermal Regime</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="page-top">
        ...
        <div class="action-bar">
          <div class="action-bar-left">
            ...
            <button class="btn-reset-soft" onclick="resetAll()"><span>↺</span> Reset semua</button>
          </div>
...
function fillReview(){{
  var mr=['Dry','Moderate','Wet'],tr=['Cold','Temperate','Hot'];
...
function resetAll(){{
  ...
  var d={{'f-bd':'1.20','f-sal':'1.00','f-om':'5.00','f-buf':'5.00','f-cec':'10.00','f-sm':'30.00','f-li':'800','f-st':'25.00','f-ph':'6.50','f-at':'28.00','f-nb':'0.00'}};
  Object.keys(d).forEach(function(id){{document.getElementById(id).value=d[id];}});
  ...
  updateNav(-1); cur='landing';
}}

function saveToLocal(){{
  var data={{bulk_density:gv('f-bd'),organic_matter_pct:gv('f-om'),cation_exchange_capacity:gv('f-cec'),salinity_ec:gv('f-sal'),buffering_capacity:gv('f-buf'),soil_moisture_pct:gv('f-sm'),soil_temp_c:gv('f-st'),air_temp_c:gv('f-at'),light_intensity_par:gv('f-li'),soil_ph:gv('f-ph'),moisture_regime:segs.mr,thermal_regime:segs.tr,nutrient_balance:gv('f-nb')}};
  try{{localStorage.setItem('agro_data',JSON.stringify(data));alert('Data tersimpan!');}}catch(e){{}}
}}
...
window.addEventListener('load',function(){{
  try{{
    ...
    if(saved){{
      ...
      var map={{'f-bd':'bulk_density','f-om':'organic_matter_pct','f-cec':'cation_exchange_capacity','f-sal':'salinity_ec','f-buf':'buffering_capacity','f-sm':'soil_moisture_pct','f-st':'soil_temp_c','f-at':'air_temp_c','f-li':'light_intensity_par','f-ph':'soil_ph','f-nb':'nutrient_balance'}};
      Object.keys(map).forEach(function(id){{if(d[map[id]]!==undefined) document.getElementById(id).value=d[map[id]];}});
...
async function runPredict(){{
  ...
  var payload={{bulk_density:gv('f-bd'),organic_matter_pct:gv('f-om'),cation_exchange_capacity:gv('f-cec'),salinity_ec:gv('f-sal'),buffering_capacity:gv('f-buf'),soil_moisture_pct:gv('f-sm'),soil_temp_c:gv('f-st'),air_temp_c:gv('f-at'),light_intensity_par:gv('f-li'),soil_ph:gv('f-ph'),moisture_regime:segs.mr,thermal_regime:segs.tr,nutrient_balance:gv('f-nb')}};
  ...
  try{{
    ...
    if(res.ok){{
      ...
      if('prediction' in data){{
        var conf=data.confidence?' <span style="font-size:11px;opacity:.65">(confidence: '+(data.confidence*100).toFixed(1)+'%)</span>':'';
      ...
      area.innerHTML='<div class="result-box result-no"><div class="result-icon">!</div><div><div class="r-title">Backend tidak merespon</div><div class="r-sub">Pastikan server FastAPI berjalan: uvicorn app:app --reload</div></div></div>';
    }}
  }}catch(e){{
    area.innerHTML='<div class="result-box result-no"><div class="result-icon">!</div><div><div class="r-title">Tidak dapat terhubung ke backend</div><div class="r-sub">'+e.message+'</div></div></div>';
  }}
  btn.disabled=false;
